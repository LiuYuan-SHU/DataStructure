...
//	辅助函数:
	...
	void PostRootOrder(ChildSiblingTreeNode<ElemType> *r, void (*Visit)(const ElemType &)) const;
		// 后根遍历以r为根的树(请在下面补充完整代码） 
	int NodeCount(ChildSiblingTreeNode<ElemType> *r) const;	// 求以r为根的树的结点个数
	int Height(ChildSiblingTreeNode<ElemType> *r) const;	// 求以r为根的树的高(请在下面补充完整代码） 
	int BHeight(ChildSiblingTreeNode<ElemType> *r) const;	// 求以r位根的二叉树的高 
...
//  孩子兄弟表示树类方法的声明:
	...
	//void PreRootOrderWithStack(void (*Visit)(const ElemType &)) const;		// 树的先根序遍历
...
{	
	ChildSiblingTreeNode<ElemType>* p=new ChildSiblingTreeNode<ElemType>;  //将森林生成为一颗树
...
/////////////////////////////补充完整代码//////////////////////////
	...
	if (r != NULL) {
		for (p = FirstChild(r); p != NULL; p = NextSibling(p))
			PostRootOrder(p, Visit);// 依次先根序遍历根结点的每棵子树
			(*Visit)(r->data);			// 访问根结点
	}
}
